...
export default function LoginPage() {
  ...
  const [forgotAnimating, setForgotAnimating] = useState(false);
  ...
  useEffect(() => {
    if (showForgot) {
      setForgotAnimating(true);
      const timer = setTimeout(() => setForgotAnimating(false), 300);
      return () => clearTimeout(timer);
    }
  }, [showForgot]);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4" style={{ backgroundColor: '#0f172a' }}>
      <main 
        ...
        <nav className="flex rounded-xl p-1 mb-8" style={{ backgroundColor: 'rgba(15, 23, 42, 0.5)', border: '1px solid #334155' }}>
          <button 
            className="flex-1 py-2.5 text-sm font-bold rounded-lg transition-all duration-300 ease-out"
            style={{ 
              backgroundColor: activeTab === 'USER' ? '#fbbf24' : 'transparent',
              color: activeTab === 'USER' ? '#0f172a' : '#94a3b8',
              transform: activeTab === 'USER' ? 'scale(1.02)' : 'scale(1)',
              boxShadow: activeTab === 'USER' ? '0 0 20px rgba(251, 191, 36, 0.3)' : 'none'
            }}
          ...
          <button 
            className="flex-1 py-2.5 text-sm font-bold rounded-lg transition-all duration-300 ease-out hover:text-white"
            style={{ 
              backgroundColor: activeTab === 'ADMIN' ? '#fbbf24' : 'transparent',
              color: activeTab === 'ADMIN' ? '#0f172a' : '#94a3b8',
              transform: activeTab === 'ADMIN' ? 'scale(1.02)' : 'scale(1)',
              boxShadow: activeTab === 'ADMIN' ? '0 0 20px rgba(251, 191, 36, 0.3)' : 'none'
            }}
        ...
        <form key={activeTab} onSubmit={handleSubmit} className="space-y-6" style={{ animation: 'fadeIn 0.4s ease-out' }}>
          {error && (
            ...
            <div className="mt-8">
              <div className="relative">
                ...
                <div className="relative flex justify-center text-[10px] uppercase font-bold tracking-widest">
                  <span className="px-2" style={{ backgroundColor: '#1e293b', color: '#64748b' }}>Or continue with</span>
                </div>
              </div>
              <div className="mt-6">
                {googleLoading ? (
        ...
        {activeTab === 'ADMIN' && (
          <p className="mt-6 text-center" style={{ color: '#64748b', fontSize: '13px' }}>
        ...
        <div className="mt-8 text-center">
          <p className="text-sm" style={{ color: '#64748b' }}>
            ...
            <Link className="font-bold text-amber-400 hover:text-amber-300 transition-colors ml-1" to="/register">
              Create an Account
            </Link>
          </p>
        </div>
        ...
        >
          <div 
            ...
            style={{ 
              ...
              border: '1px solid #334155',
              transform: forgotAnimating ? 'scale(0.9)' : 'scale(1)',
              opacity: forgotAnimating ? 0 : 1,
              animation: forgotAnimating ? 'none' : 'fadeIn 0.3s ease-out'
            }}
            onClick={(e) => e.stopPropagation()}
          >
            <h2 className="font-bold text-xl text-white text-center mb-4">🔑 Reset Password</h2>
            
            {forgotSuccess ? (
              <div key="success" style={{ animation: 'fadeIn 0.4s ease-out' }}>
              ...
              <div key="form" style={{ animation: 'fadeIn 0.4s ease-out' }}>
                ...
                <button 
                  className="w-full text-center mt-3 text-sm font-bold text-amber-400 hover:text-amber-300"
